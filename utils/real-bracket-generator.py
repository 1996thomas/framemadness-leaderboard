...
def load_progress(file_path='tournament_progress.json'):
    try:
...
def select_winner(team1, team2):
    ...
    while True:
        ...
        if choice == 'quit':
            ...
            print("Invalid selection. Please enter 1 or 2, or type 'quit' to save and exit.")
...
def main():
    ...
    while len(teams) > 1:
        ...
        for i in range(0, len(teams), 2):
            ...
            if team2:
                ...
                if winner is None:  # Quitter
                    save_progress(progress + new_progress)
                    print("Progress saved. You can resume later.")
                    sys.exit()
                next_round_teams.append(winner)
                winner_key = list(load_teams().keys())[list(load_teams().values()).index(winner)]
                new_progress.append({"m": current_match_id, "w": int(winner_key)})  # Convertir l'ID de l'équipe en entier
                current_match_id += 1
            else:
    ...
    save_progress(progress + new_progress)
    print("Tournament completed. Final results saved.")
